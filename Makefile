...
# Additional ARCH settings for x86 (32-bit fork: i386 only)
ifeq ($(ARCH),i386)
        SRCARCH := x86
endif
ifeq ($(ARCH),x86_64)
$(error This is a 32-bit-only kernel fork. Use ARCH=i386 or ARCH=x86 with CONFIG_64BIT=n.)
endif

# Additional ARCH settings for sparc (32-bit fork: sparc32 only)
ifeq ($(ARCH),sparc32)
       SRCARCH := sparc
endif
ifeq ($(ARCH),sparc64)
$(error This is a 32-bit-only kernel fork. Use ARCH=sparc32 or ARCH=sparc with CONFIG_64BIT=n.)
endif
...
# 32-bit fork: reject removed 64-bit-only architectures
ifeq ($(SRCARCH),$(filter $(SRCARCH),arm64 loongarch s390 alpha))
$(error This is a 32-bit-only kernel fork. Architecture '$(SRCARCH)' is not supported.)
endif
